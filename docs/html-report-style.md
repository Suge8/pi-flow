...
- HTML 由插件渲染：`src/goal/html.ts`、`src/flow/html.ts`、`src/shared/report-blocks.ts`。
- HTML 只展示状态，不保存事实；事实来自 `goal.json` / `flow.json` 和计划 Markdown。
- 渲染前必须校验状态源；校验失败页只用于排查，不自动打开半成品。
- 模型禁止写 `goal.html` / `flow.html`。

## 内容

- HTML 是给用户看的监控页，不是给执行 AI 看的计划。
- 直接展示：标题、`Objective` 首句、任务清单、完成验收、质量检查、成果首段。
...
- 静态 HTML：Tailwind CDN + rough.js CDN；不引入应用代码。
