...
- Flow 最终卡片与报告 status 仍给精确 capability live report URL；服务根路径另提供匿名目录页（Live / Recent），只列出曾经认证注册的 Flow / 独立质检报告，按完整 cwd 辨认并点入 `/r/<cap>/`，返回依赖浏览器导航。独立质检 HTML 每次写盘成功后立即发布 status URL，启动卡、轮次结果卡和终态通知均不重复展示；完成后重开同一 session 仍按已有 HTML 恢复 status 入口。默认不展示本地 HTML/Markdown 路径。
- 目录页复用 `pageShell` 主题、响应式 viewport、CSP 与相对 `EventSource("events")` reload；Live 在前、Recent 在后，按 `updatedAt` 倒序，完整 cwd 可换行，unavailable 不渲染死链接。无搜索、筛选、项目控制或任意文件浏览。
