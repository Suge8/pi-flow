...
- 状态单一事实源：运行态为 `flow.json`（schema v8）；生成前子阶段为同目录 `alignment.json`。模型只写 `flow.semantic.json` 与 markdown；插件 builder 组装 canonical 状态。
...
- 同一 cwd 可有多个 active Flow（`aligning/generating/running`）；用户提示使用裸 id（如 `/flow continue F4`）。无 id 命令只在当前 session 归属或唯一 active 时路由，多义必须要求显式 id；pre-draft 归属看 `alignment.json.sessionFile`，running 归属看当前 running Goal 的 `sessionFile`。
- pre-draft `/flow status` 只输出文本，不生成 HTML；`/flow continue F<N>` 可跨 session 接管 `alignment.json.sessionFile`，等待输入阶段恢复活动卡，`aligning/generating` 重发隐藏 prompt；完成 auto-start 只使用记录的 command context，无可用 `newSession` 时保持 draft 并提示 `/flow start F<N>`。
- Flow 写操作、active parallel batch 和 live report watcher 都按 Flow dir 作用域管理，禁止退回 cwd 单例。
- worker 没有用户可调用的公开子命令；父 session 通过私有 bootstrap 启动，崩溃恢复入口是 `/flow continue`。
