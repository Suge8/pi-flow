...
| `src/flow/target.ts` | Flow 命令目标解析：显式 id、当前 session、唯一 running 与多义提示 |
...
- 状态单一事实源：`flow.json`（schema v7）。模型只写 `flow.semantic.json` 与 markdown；插件 builder 组装 canonical 状态。
...
- 新运行目录直接写 `.flow/F<N>-slug/`；并行 worker 内部产物为 `workers/Gx/session.jsonl`、`plan.md`、`state.json`、`result.json`，不生成 step-level HTML 报告。
- 同一 cwd 可有多个 running Flow；用户提示优先短 id（如 `/flow continue F4`）。无 id 命令只在当前 session 归属或唯一 running 时路由，多义必须要求显式 id。
